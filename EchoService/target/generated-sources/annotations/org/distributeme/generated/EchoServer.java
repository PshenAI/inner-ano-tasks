...
public class EchoServer{
	...
	private static RemoteEchoService skeleton = null;
	private static RemoteEchoService rmiServant = null;
	...
	public static void createServiceAndRegisterLocally(int customRegistryPort) throws Exception{
		...
		try{
			Class<ServiceFactory<org.distributeme.EchoService>> factoryClazz = (Class<ServiceFactory<org.distributeme.EchoService>>)Class.forName("org.distributeme.EchoServiceFactory");
			MetaFactory.addFactoryClass(org.distributeme.EchoService.class, Extension.LOCAL, factoryClazz);
		}catch(ClassNotFoundException factoryNotFound){
			try{
				// Even more convenient - try to instantiate the implementation directly
				Class<? extends org.distributeme.EchoService> implClazz = (Class<? extends org.distributeme.EchoService>)Class.forName("org.distributeme.EchoServiceImpl");
				MetaFactory.createOnTheFlyFactory(org.distributeme.EchoService.class, Extension.LOCAL, implClazz.newInstance());
			}catch(ClassNotFoundException implNotFound){
				log.info("Giving up trying to find an impl instance, tried org.distributeme.EchoServiceFactory and org.distributeme.EchoServiceImpl, expect start to fail since init code were empty too and no factory has been supplied explicitely");
			} //...inner catch
		} //...outer catch
		org.distributeme.EchoService impl = null;
		try{
			impl = MetaFactory.get(org.distributeme.EchoService.class, Extension.LOCAL);
		}catch (FactoryNotFoundException factoryNotFound){
			throw new AssertionError("Un- or mis-configured, can't instantiate service instance for org.distributeme.EchoService tried initcode, submitted factory, autoguessed factory (org.distributeme.EchoServiceFactory) and impl class (org.distributeme.EchoServiceImpl)");
		}
		skeleton = new RemoteEchoServiceSkeleton(impl);
		rmiServant = (RemoteEchoService) UnicastRemoteObject.exportObject(skeleton, SystemProperties.SERVICE_BINDING_PORT.getAsInt());
