...
outlog=${outlog:=$logdir/nohup-HelloWorldServer-out.log}
errlog=${errlog:=$logdir/nohup-HelloWorldServer-err.log}
tmpdir=$release_path/${tmpdir:=/tmp}
pidfile=${pidfile:=$tmpdir/HelloWorldServer.pid}

server='org.distributeme.helloworld.generated.HelloWorldServer'
...
start() {
	...
	running=$?
	if [ $running == 0 ]; then
		echo -n 'Starting HelloWorldServer... '
		...
		echo HelloWorldServer is already running
	fi
}
...
stop() {
	...
	running=$?
	if [ $running == 0 ]; then
		echo 'HelloWorldServer is not running'
	else
		echo -n 'Stopping HelloWorldServer... '
		pid=`cat "$pidfile"`
		kill $pid
		sleep 2
		isServerUp
...
status() {
	...
	running=$?
	if [ $running == 1 ]; then
		echo 'HelloWorldServer is running'
	else
		echo 'HelloWorldServer is not running'
	fi
}
